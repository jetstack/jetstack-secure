...
// This file holds the legacy CyberArk Identity username/password (UP) login.
// It is retained alongside the Conjur JWT exchange for backward compatibility:
// the product is GA and existing installs authenticate with ARK_USERNAME/
// ARK_SECRET. The agent selects UP vs Conjur by config presence (see
// internal/cyberark/client.go) — a Conjur service-id, when set, takes
// precedence. Both paths populate the same token cache and satisfy
// RequestAuthenticator via AuthenticateRequest.
